<template>
	<view class="page">
		...
		<view class="user-flotter">
			<view :style="'position: fixed;width: 100%;height: '+listTwoHeight+'px;background-color: #FFFFFF;'" v-if="loadShow">
				<!-- loading加载动画 -->
				<loadingt></loadingt>
			</view>
			<!-- 作品 -->
			<view v-if="navType == 0">
				<view class="user-works f">
					<view class="user-works-item" :id="item.vid" :ref="item.vid" v-for="(item,index) in vodImgList" :key="item.index" @click="videoSee(index)">
...
<script>
	...
	export default{
		...
		onPageScroll(e) {
			this.balckShow = e.scrollTop >= 5 ? true : false
			this.balckOpacity = e.scrollTop >= 100 ? true : false
		},
		onReachBottom() {
			this.loadingData()
		},
